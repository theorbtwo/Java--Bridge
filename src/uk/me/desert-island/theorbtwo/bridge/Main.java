  ...
  private static void handle_line(StringBuilder in_line, PrintStream out, PrintStream err) {
    ...
    if (command_string.equals("create")) {
      ...
      try {
        ...
        out.print("thrown: " + e.toString());
        return;
      }

    } else if (command_string.equals("DESTROY")) {
      known_objects.remove(split[1]);
      out.printf("DESTROYed\n");
      ...
      out.printf("call_method return: %s\n", obj_ident(ret));
      ...
      try {
        klass = Class.forName(split[1]);
        ret = klass.getMethod(split[2]).invoke(null);
      } catch (java.lang.Throwable e) {
        out.printf("thrown: %s\n", e.toString());
        return;
      }

      out.printf("call_static_method return: %s\n", obj_ident(ret));
      known_objects.put(obj_ident(ret), ret);
      ...
      try {
        klass = Class.forName(split[1]);
        ret = klass.getField(split[2]).get(null);
      } catch (java.lang.Throwable e) {
        out.printf("thrown: %s\n", e.toString());
        return;
      }
      ...
      Object obj = known_objects.get(split[1]);
